  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="text-center mb-10">
        ...
        <p className="text-slate-400">Calculate your monthly EMI, total interest, and total payment instantly.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="glass-panel p-8 rounded-2xl shadow-xl border border-white/10">
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-2">Loan Amount (Principal)</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-800 border border-slate-600 rounded-lg pl-10 pr-4 py-3 text-white focus:ring-primary focus:border-primary outline-none"
                />
              </div>
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-2">Loan Tenure (Years)</label>
              <input
                type="number"
                value={tenure}
        ...
        <div className="glass-panel p-8 rounded-2xl shadow-xl border border-white/10 flex flex-col justify-center">
          {result ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              <div className="bg-slate-800/50 p-6 rounded-xl text-center border border-emerald-500/30">
                <p className="text-slate-400 mb-1">Monthly EMI</p>
                <h2 className="text-4xl font-bold text-emerald-400">${result.emi.toFixed(2)}</h2>
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                 <div className="bg-slate-800/50 p-4 rounded-xl text-center">
                  <p className="text-slate-400 text-sm mb-1">Total Interest</p>
                  <h3 className="text-xl font-bold text-white">${result.totalInterest.toFixed(2)}</h3>
                </div>
                <div className="bg-slate-800/50 p-4 rounded-xl text-center">
                  <p className="text-slate-400 text-sm mb-1">Total Payment</p>
                  <h3 className="text-xl font-bold text-white">${result.totalPayment.toFixed(2)}</h3>
                </div>
              </div>
            </motion.div>
          ) : (
            <div className="text-center text-slate-500">
              ...
              <p>Enter your loan details and click calculate to see the breakdown.</p>
            </div>
          )}
        </div>
      </div>
