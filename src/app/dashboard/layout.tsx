...
const NAV = [
  ...
  { key: "tools",     label: "All Tools",  icon: Wrench,          href: "/dashboard?tab=tools" },
  { key: "settings",  label: "Profile",    icon: User,            href: "/dashboard?tab=settings" },
  { key: "security",  label: "Security",   icon: Shield,          href: "/dashboard?tab=security" },
];
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        ...
        {NAV.map(item => (
          <Link key={item.key} href={item.href}
            ...
            className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold text-slate-600 hover:bg-primary/5 hover:text-primary transition-all group">
      ...
      <div className="flex-1 lg:ml-64 flex flex-col min-h-screen">
        ...
        <header className="sticky top-0 z-20 bg-white border-b border-slate-100 px-4 lg:px-8 h-14 flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-sm font-black text-slate-900">My Dashboard</h1>
              <p className="text-[11px] text-slate-400 hidden sm:block">Welcome back, {user?.name}</p>
            </div>
          </div>
