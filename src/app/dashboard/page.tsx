...
const CAT_GRADIENTS: Record<string, string> = {
  Image: "from-purple-500 to-pink-600", PDF: "from-red-500 to-orange-500",
  SEO: "from-emerald-500 to-teal-600", Developer: "from-blue-500 to-indigo-600",
  Calculator: "from-green-500 to-emerald-600", Utility: "from-yellow-500 to-orange-500",
  "AI Tools": "from-pink-500 to-rose-600", Navigation: "from-slate-500 to-slate-600",
};
...
export default function DashboardPage() {
  const { tab, user } = useDashboardTab();
  ...
  useEffect(() => {
    if (tab === "overview") {
      ...
      fetch("/api/analytics/my-usage").then(r => r.json())
      ...
      {tab === "overview" && (
        <motion.div key="overview" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-6">
          ...
          <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
            ...
            {usageLoading ? (
              <div className="flex justify-center py-8"><div className="h-6 w-6 border-2 border-primary/20 border-t-primary rounded-full animate-spin" /></div>
            ) : usage.length === 0 ? (
              <div className="text-center py-10">
                <div className="bg-slate-100 w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-3"><Wrench className="h-7 w-7 text-slate-400" /></div>
                <p className="text-slate-500 font-bold">No tool usage yet</p>
                <Link href="/tools/all" className="inline-flex items-center gap-2 mt-4 bg-primary text-white font-bold px-5 py-2.5 rounded-xl text-sm hover:bg-primary-hover transition-all">
              ...
              <div className="space-y-2">
                {usage.slice(0, 10).map((item, i) => {
                  ...
                  return (
                    <Link key={i} href={item.toolPath} className="flex items-center gap-4 p-3 rounded-xl hover:bg-slate-50 transition-all group">
                      <div className={`bg-gradient-to-br ${grad} p-2.5 rounded-xl flex-shrink-0 shadow-sm`}><CatIcon className="h-4 w-4 text-white" /></div>
                      ...
                      <span className={`text-[10px] font-black px-2 py-1 rounded-lg flex-shrink-0 ${CATEGORY_COLORS[item.category] || "bg-slate-100 text-slate-500"}`}>{item.category}</span>
                      <ChevronRight className="h-4 w-4 text-slate-300 group-hover:text-primary flex-shrink-0" />
                    </Link>
                  );
                })}
              </div>
            ...
            return (
              <div key={category} className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
                <div className="flex items-center gap-3 mb-4">
                  <div className={`bg-gradient-to-br ${grad} p-2.5 rounded-xl shadow-sm`}><CatIcon className="h-5 w-5 text-white" /></div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                  {tools.map(tool => (
                    <Link key={tool.path} href={tool.path}
                      ...
                      <div className={`bg-gradient-to-br ${grad} p-2 rounded-lg flex-shrink-0 opacity-80`}><CatIcon className="h-3.5 w-3.5 text-white" /></div>
                      <span className="text-sm font-bold text-slate-700 group-hover:text-primary truncate flex-1">{tool.name}</span>
                      <ChevronRight className="h-3.5 w-3.5 text-slate-300 group-hover:text-primary flex-shrink-0" />
                    </Link>
                  ))}
                </div>
              </div>
        ...
        <motion.div key="settings" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
          <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-8 max-w-lg">
            ...
            <form onSubmit={saveProfile} className="space-y-5">
              ...
              <AnimatePresence>{pMsg && <Msg type={pMsg.type} text={pMsg.text} />}</AnimatePresence>
              <button type="submit" disabled={pSaving || !name.trim()}
                className="flex items-center gap-2 bg-primary hover:bg-primary-hover disabled:opacity-50 text-white font-black px-6 py-3 rounded-xl text-sm transition-all">
      ...
      {tab === "security" && (
        <motion.div key="security" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
          <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-8 max-w-lg">
            ...
            <p className="text-slate-400 text-sm mb-6">Keep your account secure with a strong password</p>
              ...
              {[
                ...
                <div key={f.label}>
                  ...
                  <div className="relative flex items-center">
                    ...
                    <button type="button" onClick={f.toggle} className="absolute right-4 text-slate-400 hover:text-slate-600">
                      {f.show ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                    </button>
                  </div>
                </div>
              ))}
              <AnimatePresence>{sMsg && <Msg type={sMsg.type} text={sMsg.text} />}</AnimatePresence>
